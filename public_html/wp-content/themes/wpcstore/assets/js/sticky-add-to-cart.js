...
( function () {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		if ( typeof wpcstore_sticky_add_to_cart_params === 'undefined' ) {
			return;
		}

		const trigger = document.getElementsByClassName(
			wpcstore_sticky_add_to_cart_params.trigger_class
		);
			...
			const stickyAddToCartToggle = function () {
				if (
					...
					stickyAddToCart[ 0 ].classList.add(
						'wpcstore-sticky-add-to-cart--slideInDown'
					);
					stickyAddToCart[ 0 ].classList.remove(
						'wpcstore-sticky-add-to-cart--slideOutUp'
					);
				} else if (
				...
				if ( product ) {
					if (
						...
						const selectOptions = document.getElementsByClassName(
							'wpcstore-sticky-add-to-cart__content-button'
						);
