...
function wactc_add_to_cart_text_filter( $text, $product ){
    ...
    $default_text = $current['icon'] == 'only_icon' ? false : $text;
    $type = $product->get_type();
    
    $return = !empty( $current[$type] ) ? $current[$type] : $default_text;
    return  $return;
}
...
function wactc_customized_style_in_head(){
    ...
    .single_add_to_cart_button.button:<?php echo $after_before; ?>,.add_to_cart_button:<?php echo $after_before; ?>{
        content: "\e01d";
        z-index: 99;
    ...
    if( $saved_data['icon'] == '' && isset( $saved_data['cart_icon_preset'] )&& !empty( $saved_data['cart_icon_preset'] ) ){
        ...
        $url = WACTC_BASE_URL . 'images/' . $name . '.png';
        $width = $height = 20;
        $image_bool = true;
        ...
        $url = $image['url'];
        $width = isset( $image['width'] ) && !empty( $image['width'] ) ? $image['width'] : 20;
        $height = isset( $image['height'] ) && !empty( $image['height'] ) ? $image['height'] : 20;
        $image_bool = true;
    }
